...
#model 1
@time solution1 = Ensembles.run_ensemble_standard_output(sim1, (0.0, 2500.0), selection1; trajectories=traj_num,
    output=(:population), saveat=rng)
plt = plot(legend=false)
global avg = [zeros(2) for _ in 1:251] # zero(solution1[1].population)
for s in solution1
    global avg += s.population
    #plot!(rng, [p[2] for p in s.population], label="FSSH P2", linestyle=:dash)
end
avg = avg/traj_num
...
@time solution3 = Ensembles.run_ensemble_standard_output(sim3, (0.0, 2500.0), selection3; trajectories=traj_num,
    output=(:population), saveat=rng)
global avg = [zeros(2) for _ in 1:251] #zero(solution3[1].population)
for s in solution3
