...
simulation processes that depend on model quantities like the total energy derivative or the nonadiabatic 
coupling terms.
These `caches` are updated once per time-step during the dynamics simulation and are then subsequently used as a local
address in memory that contains all of the relevant quantities for a given dynamics method.

!!! note "What is the purpose of NQCCalculators?"
    `NQCCalculators.jl` formerly existed as a part of `NQCDynamics.jl` where it was conceived primarily as a way of
    calculating and caching relevant quantities for dynamics methods. While it continues to serve this purpose
    as a separate module, it can now be more flexibly used in conjunction with `NQCModels.jl` as a collection of
    pre-implemented methods for calculating the properties and observables of systems defined using the `NQCModels.jl` package.
    Examples of this use case are given below.
...
`model` object to build a `cache` that can calculate, manipulate and store more advanced properties of the system.
...
Here we have created an empty cache with all of the data structures necessary to contain quantities that define 3 independent atoms 
...
of atomic coordinates associated with positions within that electronic structure.
...
- the `nonadiabatic coupling` matrices
- a temporary matrix for ad-hoc assignment during calculations

!!! info "Not all Caches store the same data"
    In general, the data the `cache` stores depends on the type of `model` and whether or not the atoms are being modelled as `ring polymers`.
    ...
    system. More details about what data each type of `cache` contains can be found in the relevant section of the `NQCCalculators` docs.
...
coordinates and then populates the `cache` object we passed in with all of the values associated with that set of coordinates. 
...
If, however, we are only interested in calculating a specific datapoint at a given position, we can use the `update_\$(quantity)!` functions.
...
!!! info "update!" vs evaluate" functions
    Notice how the `update_\$(quantity)!` functions have an exclimation point, `!`, whereas the `evaluate_\$(quantity)` functions do not. 
    This is common Julia notation for indicating an **in-place function**. 
    An in-place function does not return a variable but instead updates one of the provided arguments "in-place". 
    ...
    without returning a new variable. For the `evaluate_\$(quantity)` functions however, we want a new variable to be returned and for the cache in 
    its original state to be preserved, so regular (non-in-place) functions are used (indicated by no `!` in the name) 
    that return new variables containing the "evaluated" quantity. 
