...
atoms = Atoms([:H, :H])
sim = Simulation{MDEF}(atoms, NonadiabaticModels.FreeConstantFriction(atoms.masses[1]); temperature=10u"K", DoFs=2)
...
sim = Simulation{MDEF}(atoms, NonadiabaticModels.FrictionHarmonic(); DoFs=2, temperature=f)
sol = Dynamics.run_trajectory(u, (0.0, 100.0), sim; dt=1)
