...
z = Phasespace(fill(1.0, 1, length(atoms)), zeros(1, length(atoms)))

solution = Dynamics.run_trajectory(z, (0.0, 300), sim; dtmax=1e-2)
Rs = vcat(get_positions.(solution.u)...)
@test !isapprox(minimum(Rs), 0, atol=1e-4) | (minimum(Rs) >= 0) # check atom leaves cell

solution = Dynamics.run_trajectory(z, (0.0, 300), sim; callback=CellBoundaryCallback, dtmax=1e-2)
Rs = vcat(get_positions.(solution.u)...)
