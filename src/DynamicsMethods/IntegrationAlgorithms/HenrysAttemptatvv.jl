...
struct VerletfromHenry <: OrdinaryDiffEqPartitionedAlgorithm end

alg_order(alg::VerletfromHenry) = 2

@cache struct VerletfromHenryCache{uType,vType,rateType, uEltypeNoUnits} <: OrdinaryDiffEq.OrdinaryDiffEqMutableCache
    u::uType
    uprev::uType
    tmp::uType
    vtmp::vType
...
struct VerletfromHenryConstantCache{uEltypeNoUnits} <: HamiltonConstantCache
    half::uEltypeNoUnits
end
...
function alg_cache(alg::VerletwithHenry, u, rate_prototype, ::Type{uEltypeNoUnits}, ::Type{uBottomEltypeNoUnits}, ::Type{tTypeNoUnits}, uprev, uprev2, f, t, dt, reltol, p, calck, ::Val{true}) where {uEltypeNoUnits, uBottomEltypeNoUnits, tTypeNoUnits}
    tmp = zero(u)
    vtmp = zero(DynamicsUtils.get_velocities(u))
    k = zero(DynamicsUtils.get_positions(rate_prototype))
    fsalfirst = zero(DynamicsUtils.get_positions(rate_prototype))
    half = uEltypeNoUnits(1 // 2)
    VerletfromHenry(u, uprev, tmp, vtmp, k, fsalfirst, half)
end

function alg_cache(alg::VelocityVerlet, u, rate_prototype, ::Type{uEltypeNoUnits}, ::Type{uBottomEltypeNoUnits}, ::Type{tTypeNoUnits}, uprev, uprev2, f, t, dt, reltol, p, calck, ::Val{false}) where {uEltypeNoUnits, uBottomEltypeNoUnits, tTypeNoUnits}
    VerletfromHenryConstantCache(uEltypeNoUnits(1 // 2))
end

function OrdinaryDiffEq.initialize!(integrator, cache::VerletfromHenryCache)
