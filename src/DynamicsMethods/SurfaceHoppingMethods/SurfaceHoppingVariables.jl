...
"""
struct SurfaceHoppingVariables{T, A <: RecursiveArrayTools.RecursiveArrayTools.ArrayPartition{T}, NT <: NamedTuple} <: AbstractVector{T}
    array_partition::A
    names_to_indices::NT
end
...
# return RecursiveArrayTools.ArrayPartition when possible, otherwise next best thing of the correct size
...
function Base.zero(x::SurfaceHoppingVariables{T, S, TN}) where {T, S, TN}
    SurfaceHoppingVariables{T, S, TN}(zero(RecursiveArrayTools.ArrayPartition(x)), getfield(x, :names_to_indices))
end
...
function Base.getproperty(x::SurfaceHoppingVariables, s::Symbol)
    getindex(RecursiveArrayTools.ArrayPartition(x).x, getproperty(getfield(x, :names_to_indices), s))
end
...
@inline function Base.setproperty!(x::SurfaceHoppingVariables, s::Symbol, v)
    ...
    RecursiveArrayTools.ArrayPartition(x).x[index] .= v
end
...
Base.getindex(x::SurfaceHoppingVariables, args...) = getindex(RecursiveArrayTools.ArrayPartition(x), args...)

Base.setindex!(x::SurfaceHoppingVariables, args...) = setindex!(RecursiveArrayTools.ArrayPartition(x), args...)
function Base.map(f, x::SurfaceHoppingVariables)
    SurfaceHoppingVariables(map(f, RecursiveArrayTools.ArrayPartition(x)), getfield(x, :names_to_indices))
end
...
# when broadcasting with RecursiveArrayTools.ArrayPartition + another array type, the output is the other array tupe
...
@inline RecursiveArrayTools.unpack(x::SurfaceHoppingVariables, i) = RecursiveArrayTools.unpack(RecursiveArrayTools.ArrayPartition(x), i)

function Base.copy(A::SurfaceHoppingVariables{T, S, NT}) where {T, S, NT}
    SurfaceHoppingVariables{T, S, NT}(deepcopy(RecursiveArrayTools.ArrayPartition(A)), getfield(A, :names_to_indices))
end
...
@inline function Base.copyto!(dest::SurfaceHoppingVariables,
    ...
    @inline function f(i)
        copyto!(RecursiveArrayTools.ArrayPartition(dest).x[i], RecursiveArrayTools.unpack(bc, i))
    end
    ntuple(f, Val(N))
    return dest
end
