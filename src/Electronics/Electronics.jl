...
mutable struct ElectronicContainer{T<:AbstractFloat}
    ...
    function ElectronicContainer{T}(model::Model, n_DoF::Integer, n_states::Integer) where {T<:AbstractFloat}
        V0 = 0.0
        D0 = zeros(n_DoF)
        ...
        new(model, n_DoF, n_states, V0, D0, V, D, eigenvalues, eigenvectors, AD, NAC)
    end
end

function ElectronicContainer(model::Model, n_DoF::Integer, n_states::Integer)
    ElectronicContainer{Float64}(model, n_DoF, n_states)
end
